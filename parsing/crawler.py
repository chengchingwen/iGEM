...
def blast_info(soup):
    ctable = soup.find(class_="featureTable", id="peptides_section").contents
    for i in ctable:
        if i.find("span", class_="context-help tooltipped-click") and (i.find("span", class_="context-help tooltipped-click").contents[1] == "Peptide" or i.find("span", class_="context-help tooltipped-click").contents[1] == "Chain"):
            table = i

    key = table.find("span", class_="context-help tooltipped-click").contents[1] 
    pro = table['id']
    pos = table.find(class_="numeric").text
    return pos, key, pro


def blast_query(ID, pos, key, pro):
    blast = "http://www.uniprot.org/blast/?about=%s[%s]&key=%s&id=%s" % (ID, pos, key, pro)
    blast = blast.replace('\xa0–\xa0', '-')
    succeed = None
    while not succeed:
        try:
